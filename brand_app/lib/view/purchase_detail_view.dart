                              ...
                              child: Row(
                                children: [
                                  ...
                                  Container(
                                    ...
                                    decoration: BoxDecoration(
                                      color: _statusColor((data['pstatus']?.toString() ?? '')).withOpacity(0.1),
                                      borderRadius: BorderRadius.circular(20),
                                      border: Border.all(
                                        color: _statusColor((data['pstatus']?.toString() ?? '')),
                                        width: 0.8,
                                      ),
                                    ),
                                    child: Text(
                                      (data['pstatus']?.toString() ?? '').isEmpty ? '상태 없음' : (data['pstatus']?.toString() ?? ''),
                                      style: TextStyle(
                                        color: _statusColor((data['pstatus']?.toString() ?? '')),
                                        fontSize: 12,
                    ...
                    Card(
                      ...
                      child: Padding(
                        ...
                        child: Column(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                            if ((data['pstatus']?.toString() ?? '') == '주문완료' || (data['pstatus']?.toString() ?? '') == '수령대기')
                              Padding(
                                padding: const EdgeInsets.symmetric(vertical: 4),
                                child: Text('수령지점 : ${data['sname']?.toString() ?? '-'}'),
                              ),

                            if ((data['pstatus']?.toString() ?? '') == '수령완료' ||
                                (data['pstatus']?.toString() ?? '') == '반품대기' ||
                                (data['pstatus']?.toString() ?? '') == '반품완료') ...[
                              Padding(
                                padding: const EdgeInsets.symmetric(vertical: 4),
                                child: Text('수령지점 : ${data['sname']?.toString() ?? '-'}'),
                              ),
                            ...
                            if ((data['pstatus']?.toString() ?? '') == '반품대기' || (data['pstatus']?.toString() ?? '') == '반품완료')
                              Padding(
                                padding: const EdgeInsets.symmetric(vertical: 4),
                                child: Text('반품지점 : ${data['sname']?.toString() ?? '-'}'),
                              ),

                            if ((data['pstatus']?.toString() ?? '') == '반품완료')
                              Padding(
                                padding: const EdgeInsets.symmetric(vertical: 4),
                                child: Text('반품날짜 : ${formatDate(data['refunddate'])}'),
                              ),

                            if (!((data['pstatus']?.toString() ?? '') == '주문완료' ||
                                (data['pstatus']?.toString() ?? '') == '수령대기' ||
                                (data['pstatus']?.toString() ?? '') == '수령완료' ||
                                (data['pstatus']?.toString() ?? '') == '반품대기' ||
                                (data['pstatus']?.toString() ?? '') == '반품완료'))
                              Padding(
                                padding: const EdgeInsets.symmetric(vertical: 4),
